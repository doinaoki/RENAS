...
class Rename:
    ...
    def coRename(self, idDict):
        ...
        idDict["sameFile"] = self.__calculate_file_distance(
            idDict["files"], self.__old["files"]
        )
        return self.polish_recommended_dict(idDict)

    def __calculate_file_distance(self, recFile, trigFile):
        rec_file_length = len(recFile.split("/"))
        trigger_file_length = len(trigFile.split("/"))
        common = os.path.commonpath([recFile, trigFile])
        common_file_length = len(common.split("/"))
        return rec_file_length + trigger_file_length - 2 * common_file_length
